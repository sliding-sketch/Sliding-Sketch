...
public:
	CSS() {}
	CSS(int M2, int K) :M2(M2), K(K)
	{
	    bobhash = new BOBHash32(1001);
	    clear();
    }
	void clear()
	{
		memset(Value_Index, 0, sizeof(Value_Index));
		Value_Index[0] = M2 - 1; ID_index[0].wz = -1;
		for (int i = 0; i<M2; i++)
			R[i] = i, head[i] = i + 1, ID_index[i + 1].wz = i;
	...
	int querymemory()
	{
	   ...
	   int sum=(M2+10)*4*log(M2)/log(2)+179*M2;
	   return sum;
	}
	int Hash(string ST)
	{
