...
public:
	...
	void Init(DATA_TYPE x, int time_stamp)
	{
		...
		// compute address with pseudo random algorithm
		unsigned long long base = Hash(x, 0) % M2;
		unsigned long long seed = Hash(x, 1) % M2;
		for (int j = 0; j<HK_d; j++)
		{
			//calculate position
			seed = (seed * timer + adder) % prime;
			int Hsh = (base + seed) % (M2 - (2 * HK_d) + 2 * j + 3);
